...
function countBlocksWordsLocalized(blocks, locale) {
  if (locale === "en") {
    return blocks.reduce((sum, b) => {
      if (b.type === "p") return sum + b.text.split(/\s+/).filter(Boolean).length;
      if (b.type === "ul") return sum + b.items.join(" ").split(/\s+/).filter(Boolean).length;
      return sum;
    }, 0);
  }
  return 0;
...
function buildLocaleBlocks(meta, images, locale) {
  ...
  throw new Error(`Locale "${locale}" generation is disabled — English-only publish mode`);
}
...
export function generateArticle(locale, topic, opts = {}) {
  ...
  const localizedMeta =
    locale === "en"
      ...
      : { ...topic, topicType, locale, primaryKeyword: pk };
